...
export default class TodoList extends React.Component {
  ...
  render() {
    ...
    return <div>
      <h1>todos</h1>
      <input className="new" onKeyPress={this.createNew.bind(this)} placeholder="Add a todo" />
      <input className="filter" value={filter} onChange={this.filter.bind(this)} placeholder="filter todos"/>
      <ul>{todoLis}</ul>
      <button className="btn btn-secondary" href="#" onClick={clearComplete}>Clear Complete</button>
    </div>
  }
}
